/* Oldest First sim-outorder.c - sample out-of-order issue perf simulator implementation */
...
struct RUU_station {
  ...
  unsigned int ready_cycle; /* cycle number at enqueue time */
...
/* enqueue an RUU station into the ready queue, keeping the queue
   sorted by rs->seq (oldest first: smallest rs->seq at head).
   Mode B: pure age ordering, no special-case for loads/stores. */
static void
readyq_enqueue(struct RUU_station *rs)		/* RS to enqueue */
{
  ...
  /* sanity checks */
  if (!rs)
    fatal("readyq_enqueue: NULL rs");
  if (rs->queued)
    panic("readyq_enqueue: node is already queued");
    
    
   /* NEW: record when this instruction became ready */
  rs->ready_cycle = sim_cycle;   /* <-- ADD THIS LINE HERE */

  /* allocate a ready-list node and attach rs */
  RSLINK_NEW(new_node, rs);
  new_seq = rs->seq;
  new_node->x.seq = new_seq;
  new_node->next = NULL;

  /* insert so that ready_queue remains sorted by ascending seq:
     smallest seq (oldest) at head */
  prev = NULL;
  node = ready_queue;

  while (node && node->x.seq < new_seq) {
    prev = node;
    node = node->next;
  }
...
ruu_issue(void)
{
  ...
  /* === Sort ready queue by priority: slip → ready_cycle → seq === */
{
  struct RS_link *a, *b, *prev, *next;
  int swapped;
  ...
  do {
    ...
    while (a && (b = a->next)) {
      ...
      /* compute slip (criticality) */
      int slip_a = sim_cycle - a->rs->seq;
      int slip_b = sim_cycle - b->rs->seq;

      /* compare priorities */
      int swap = 0;
      if (slip_a < slip_b)
        swap = 1;
      else if (slip_a == slip_b) {
        if (a->rs->ready_cycle > b->rs->ready_cycle)
          swap = 1;
        else if (a->rs->ready_cycle == b->rs->ready_cycle &&
                 a->x.seq > b->x.seq)
          swap = 1;
      }
  ...
  for (n_issued=0;
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
	    ...
	    } /* !store *//* === Sort ready queue by priority: slip → ready_cycle → seq === */
{
  struct RS_link *a, *b, *prev, *next;
  int swapped;
